...
Thanks for your interest. The project is small and opinionated, but useful
...
You need **Node 22+**. Supabase's realtime client requires native
`WebSocket`. Java 21 is only needed if you're touching the sibling
`verify-mod` project.

## Code style

- ES modules everywhere (`type: "module"` in package.json).
- Single quotes, two-space indent.
- Each file has a short header comment explaining what it owns.
- Logging goes through `logger.child('tag')`. Don't `console.log` outside
  of `scripts/`.
- Database access goes through `src/db/queries.js`. Don't sprinkle Supabase
  calls across feature modules.
- All server-side MC commands flow through the validated wrappers in
  `src/mineflayer/commands.js` (`mcGive`, `mcTeleport`, and friends).
  Don't reach for RCON or call `bot.chat('/...')` directly from feature
  code.
- Settings live in `src/systems/settings/defaults.js`. To add one, call
  `registerSetting(...)` once and use `getSetting('key')` anywhere.
- Discord component custom IDs follow `panel:action[:arg...]`.
...
3. **Domain logic** in `src/systems/<area>/`. Keep this layer free of
   discord.js so it can be tested in isolation later.

If your feature needs runtime tuning, add settings rather than env vars.
...
There's no automated test suite yet, so it's manual testing for now.

For most features, the smoke test loop is:

1. `npm start`
2. Trigger your feature in Discord
3. Check the bot log for warnings or errors
4. Verify the DB and MC server state match expectations

If you want to add tests, **please do**. `vitest` is a reasonable choice
since the codebase is ESM.

## Commit hygiene

- One logical change per PR. Squash unrelated commits.
- Commit messages: imperative mood, short summary on the first line, body
  for context if it helps.
- Reference the issue you're fixing (`fixes #42`) in the PR description.

## Security

Found a security issue? Please don't open a public issue or talk about it
in a public channel. DM me on Discord at **@ygghwn** and I'll get back to
you as soon as I can (usually within a day or two).
...
- The validated command wrappers (`src/mineflayer/commands.js`). If
  user-supplied input ever reaches `sendCommand` without going through a
  wrapper, that's a vulnerability.
- The blocklist and automod. Bypasses, or false-positive denial of
  service.

## Out of scope

This bot is built for one specific community shape: a small-to-medium
modded MC server with sponsor and bounty rituals. PRs that pull it toward
a generic moderation bot, a music bot, or a corporate community bot are
