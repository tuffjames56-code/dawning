...
only. Players join here first, type a 6-digit code in chat, and get
kicked back to the main server's address. They never see anything
meaningful: a tiny void world is all that's generated.
...
> Note: in code and env vars we use `VERIFY_SERVER_*` for historical
> reasons. In docs and copy, prefer "verification server".
...
| Players | A handful at a time. Nobody stays more than about 30 seconds. |
...
2. Stop the **verification server** if it's running.
...
3. Op yourself from the console, join in-game, and run these once to
   lock the spawn:
...
Minecraft version rejects the inline JSON above (look for a warning in
the server log, ending with a default flat plains world), use one of:

- A void-world datapack (registers a `the_void` preset).
- Just accept the default flat world. Adventure mode, spawn protection,
  and the worldborder still prevent players from doing anything. They're
  only here for about 30 seconds anyway.
...
- `white-list=false`: the verification server is open by design. The
  **main server** stays whitelisted, and only sponsored players land on
  it.
